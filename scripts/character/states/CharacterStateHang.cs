...
public class CharacterStateHang : ICharacterState
{
    ...
    public CharacterStateHang(CharacterData characterData, SpecialFace attachedFace = null, float rayCastBuffer = 0.0f)
    {
    ...
    public ICharacterState ChangeState()
    {
        ...
        else if (!faceOn.ClimbableEntity.Destroyed && !characterData.IsStunned() && faceOn.ChangeToClimb)
        {
            return new CharacterStateClimb(characterData, faceOn);
        }
        else if (faceOn.ClimbableEntity.Destroyed || characterData.HasLostGrip() || (!characterData.IsStunned() && !characterData.Controller.GripInput()) || (!characterData.IsStunned() && !validHangFace))
        {
